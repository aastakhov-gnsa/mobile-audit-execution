...
function AuthTest() {
    ...
    const buttons = (
        <ButtonContainer>
            {(accessToken || idToken) ?  <Button onPress={doSignOff} text="Sign Off"/> : <Button onPress={doAuthorize} text="Sign On"/>}
        </ButtonContainer>)

    const bodyContainer = (accessToken || idToken)
    ? <View style={{
            flex: 1,
            ...
            <View style={{
                flex: 3,
                flexDirection: (orientation == 'portrait') ? 'column'
                    : 'row',
                justifyContent: 'space-around'
            }}>
                <View>
                ...
                <Heading fontSize={orientation == 'portrait' ? hp('3%')
                    : hp('7%')}
                         textAlign={orientation == 'portrait'
                             ? 'center' : 'right'}

...
import {AuditStandardExecution, Option, StandardQuestion} from '../../../interfaces/standard';
import { surveyDetails } from '../../../interfaces/survey';
import {grayBackground} from './styles';

function getStandardDescription(show: boolean, question: StandardQuestion, showComments: boolean) {
    return show ? `
<div>
Description: ${question.mcDescription}
    <div>
    ${question.attachedComment && showComments ? `Comment: ${question.attachedComment}` : ''}
    </div>
</div>
` : ''
}
...
<div>
    ...
    <div>
        ${Boolean(item.attachedComment) && showComments ? `Comment: ${item.attachedComment}` : ''}
    </div>
    <div>
        ${Boolean(item.overruleComment?.value) ? `Overrule comment: ${item.overruleComment?.value}` : ''}
    </div>
</div>
` : () => ''
...
function getQuestionRow(
    ...
    <tr>
        ...
        <td style="padding-bottom: 8px; padding-top: 8px;">
            ...
            ${showDescription ? getStandardDescription(showDescription, question, showComments) : ''}
        </td>
        <td style="padding-bottom: 8px; padding-top: 8px;">
            ${isArrayQuestion
                ? ''
                : passedStatuses.includes(question.resultCd as any) ? 'Yes' : 'No'
            }
        </td>
    </tr>
...
<tr>
    ...
    <td style="padding-top: 4px; padding-bottom: 4px">
        <div style="font-weight: 700">
            ${questionOptionsIndexLetters[index]}) ${option.value}
        </div>
        Description: ${option.hint}
    </td>
    <td>
        ${!option.resultCd || failedStatuses.includes(option.resultCd) ? '' : 'Yes'}
    </td>
</tr>
`
}
